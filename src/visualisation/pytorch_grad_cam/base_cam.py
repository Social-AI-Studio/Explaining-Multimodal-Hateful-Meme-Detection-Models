...
class BaseCAM:
    ...
    def forward(self, input_tensor, target_category=None, eigen_smooth=False):
        ...
        image_tensor = input_tensor.image

        if type(target_category) is int:
            target_category = [target_category] * image_tensor.size(0)

        if target_category is None:
            target_category = np.argmax(output.cpu().data.numpy(), axis=-1)
        else:
            assert(len(target_category) == image_tensor.size(0))
        ...
        cam = self.get_cam_image(image_tensor, target_category, 
        ...
        result = []
        for img in cam:
            img = cv2.resize(img, image_tensor.shape[-2:][::-1])
            img = img - np.min(img)
            img = img / np.max(img)
            result.append(img)
